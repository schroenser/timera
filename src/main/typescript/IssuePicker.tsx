...
async function getIssuePickerIssues(query: string): Promise<Issue[]> {
    const response = await fetch(baseUrl + "/rest/timera/1.0/issuepicker?" + new URLSearchParams({
        query: query
    }), {
        headers: new Headers({
...
function IssuePicker({
    ...
    const onOptionSubmit = useCallback((issueId: string) => {
        if (issueId) {
            issues.filter(issue => issue.id.toString() === issueId).forEach(issue => {
    ...
    const options = useMemo(() => issues.map((issue) => (
        <Combobox.Option value={issue.id.toString()} key={issue.id}>
